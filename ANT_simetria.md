...
[
L_{\text{InfoNCE}} =
-\text{sim}(r, r^+) + \log \sum_{c} \exp(\text{sim}(r, c))
]
...
[
\text{ANT} = \mathbb{E}*i\left[\log \sum_j \exp(\operatorname{ReLU}(S*{11}[i,j] - m_i + \gamma))\right]
]
...
\frac{1}{2} \left( L_{\text{ANT}}(S_{11}) + L_{\text{ANT}}(S_{22}) \right)
]
...
\log \sum_{c \in N(r)} \exp(\operatorname{ReLU}(S[r,c] - m_r + \gamma))
]
