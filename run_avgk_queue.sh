...
#   • Refreshes results_report.md after every finished run.
...
# ── Configuration ─────────────────────────────────────────────────────────────
QUEUE="${QUEUE:-$SCRIPT_DIR/configs/queue_avgk.txt}"
GPUS="${GPUS:-1}"
...
LOCKFILE="${LOCKFILE:-/tmp/tagfex_avgk.lock}"
...
# ── Completion check ──────────────────────────────────────────────────────────
# A run is complete when its exp_gistlog.log contains "avg_nme1".
# Replicates _adjust_log_dir_with_loss_params() suffix logic so that each
# config maps to its own specific expected directory, preventing false positives
# when multiple experiments share the same log_dir base.
is_done() {
    local config_file="$1"
    local seed="$2"
...
try:
    ...
    sys.exit(1)  # can't check; treat as not done
...
# Build the expected base name (same logic as the else-branch in tagfex.py)
if suffix_parts:
...
# Match the specific directory plus any _v2/_v3 versioning variants
